...
int buf_init(Buffer *b) {
    b->data = (char *)malloc(CLUAJIT_BUF_INIT);
    if (!b->data) return 0;
    b->len = 0; b->cap = CLUAJIT_BUF_INIT;
    return 1;
}
void buf_free(Buffer *b) { if (b->data) free(b->data); b->data=NULL; b->len=0; b->cap=0; }
...
int buf_push(Buffer *b, const char *s, size_t n) {
    ...
    memcpy(b->data+b->len,s,n); b->len+=n; return 1;
}
int buf_pushc(Buffer *b, char c) { return buf_push(b,&c,1); }
int buf_pushs(Buffer *b, const char *s) { return buf_push(b,s,strlen(s)); }

static const char *HEX="0123456789abcdef";

static int enc_str(Buffer *b, const char *s, size_t n) {
...
static int enc_val(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys);
static int enc_table(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys) {
    if (depth>CLUAJIT_MAX_DEPTH) { buf_free(b); luaL_error(L,"json.encode: max depth exceeded"); return 0; }
    ...
    if (is_arr) {
        ...
        for (lua_Integer i=1;i<=maxn;i++) {
            ...
            else{enc_val(L,b,lua_gettop(L),depth+1,pretty,indent,sort_keys);lua_pop(L,1);}
            first=0;
        }
    ...
    for(int ki=0;ki<ns;ki++){
        ...
        if(pretty){buf_pushc(b,'\n');buf_pushs(b,pi);}
        enc_str(b,keys[ki],strlen(keys[ki]));
        pretty?buf_pushs(b,": "):buf_pushc(b,':');
        enc_val(L,b,lua_gettop(L),depth+1,pretty,indent,sort_keys);
        lua_pop(L,1);first=0;
    }
    if(pretty&&!first){buf_pushc(b,'\n');buf_pushs(b,po);}
    buf_pushc(b,'}');
...
static int enc_val(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys) {
    ...
    switch(t){
        ...
        case LUA_TBOOLEAN: return buf_pushs(b,lua_toboolean(L,idx)?"true":"false");
        case LUA_TNUMBER: {
        ...
        case LUA_TSTRING:{size_t n;const char*s=lua_tolstring(L,idx,&n);return enc_str(b,s,n);}
        case LUA_TTABLE:{lua_pushvalue(L,idx);return enc_table(L,b,lua_gettop(L),depth,pretty,indent,sort_keys);}
        default: buf_free(b);luaL_error(L,"json.encode: unsupported type: %s",lua_typename(L,t));return 0;
    }
}
...
static int parse_num(lua_State *L, Parser *p){
    ...
    char tmp[64];size_t nl=p->pos-start;if(nl>=sizeof(tmp))luaL_error(L,"json.decode: number too long at %d",(int)start);
...
static int parse_arr(lua_State *L, Parser *p, int depth){
    if(depth>CLUAJIT_MAX_DEPTH)luaL_error(L,"json.decode: max depth exceeded");
...
static int l_encode(lua_State *L){
    ...
    Buffer b;if(!buf_init(&b))luaL_error(L,"json.encode: out of memory");
    enc_val(L,&b,1,0,pretty,indent,sort_keys);
...
static const luaL_Reg json_lib[]={{"encode",l_encode},{"decode",l_decode},{NULL,NULL}};

#if LUA_VERSION_NUM>=502
int luaopen_json(lua_State *L){luaL_newlib(L,json_lib);return 1;}
#else
int luaopen_json(lua_State *L){luaL_register(L,"json",json_lib);return 1;}
#endif
