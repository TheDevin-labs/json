...
int buf_grow(Buffer *b, size_t need) {
    size_t nc=b->cap; while(nc<b->len+need)nc*=2; if(nc==b->cap)return 1;
    char *nd=(char*)realloc(b->data,nc); if(!nd)return 0; b->data=nd; b->cap=nc; return 1;
}
int buf_push(Buffer *b, const char *s, size_t n) { if(!buf_grow(b,n))return 0; memcpy(b->data+b->len,s,n); b->len+=n; return 1; }
...
static int enc_val(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys, int superstring);

static int enc_table(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys, int superstring) {
    ...
    for (int ki=0;ki<ns;ki++) {
        ...
        if (!first&&!buf_pushc(b,',')) { lua_pop(L,1); goto cleanup; }
        if (pretty) { buf_pushc(b,'\n'); buf_pushs(b,pi); }
...
static int enc_val(lua_State *L, Buffer *b, int idx, int depth, int pretty, int indent, int sort_keys, int superstring) {
    switch(lua_type(L,idx)) {
        ...
        case LUA_TNUMBER: {
            ...
            if (lua_isinteger(L,idx)) { char t[32]; snprintf(t,sizeof(t),"%lld",(long long)lua_tointeger(L,idx)); return buf_pushs(b,t); }
#endif
...
static int parse_obj(lua_State *L, Parser *p, int depth) {
    ...
    while(1){
        skip_ws(p);
        if(p->pos>=p->len||p->s[p->pos]!='"')luaL_error(L,"json.decode: expected key at %d",(int)p->pos);
        parse_str(L,p);
